...
"use client"; // 🚨 Ajout indispensable pour utiliser usePathname()

import Link from "next/link";
import { usePathname } from "next/navigation"; // 🚨 Le hook magique
import {
  Calendar,
  FileText,
  LayoutDashboard,
...
import { logoutUser } from "@/actions/auth"; // 🚨 Appel de votre Server Action existante
import { cn } from "@/lib/utils";

interface OrganizerSidebarProps {
  // currentPath a été retiré des props car on le calcule en interne
  user: {
    nom: string;
    prenom: string;
    avatarUrl: string | null;
...
export function OrganizerSidebar({
  ...
  // Récupération dynamique du chemin actuel (ex: "/organizer/conferences")
    ...
    <aside className="fixed left-0 top-0 z-40 flex h-screen w-64 flex-col bg-slate-900">
      ...
      >
        ...
        {NAV_ITEMS.map((item) => {
          ...
          const isActive =
            item.matchPrefix === "/organizer/dashboard"
              ? currentPath === "/organizer/dashboard" // Match exact pour l'accueil
              : currentPath.startsWith(item.matchPrefix ?? item.href); // Sous-pages incluses
      ...
      {/* Logout — appel de l'action serveur importée */}
      <div className="border-t border-white/10 p-3">
